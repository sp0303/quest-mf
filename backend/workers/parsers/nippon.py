...
class NipponIndiaParser(BaseAMCParser):
    ...
    def parse_workbook(
        ...
        as_of_date: date,
        disclosed_date: date,
        scheme_name_filter: str | None = "Small Cap",
    ) -> ParsingResult:
        """Parse Nippon India Excel workbook into validated holdings list."""
        ...
        target_sheet = None
        filter_lower = (scheme_name_filter or "").lower()

        # 1. Match sheet by name or first sheet if single-sheet
        ...
        if target_sheet is None:
            # Fall back to active sheet or first sheet
        ...
        if not rows_data:
            return ParsingResult(
                ...
                equity_count=0,
                valid=False,
                validation_message="Sheet is empty",
            )
        ...
        for row in rows_data[start_row:]:
            ...
            # Ignore subtotal, header, and footer rows
            upper_name = name_str.upper()
            if any(k in upper_name for k in ("TOTAL", "SUB TOTAL", "GRAND TOTAL", "PORTFOLIO AS ON", "NOTES:", "DISCLAIMER")):
                continue
